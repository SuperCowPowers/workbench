...
# Artifacts whose name contains any of these substrings are never flagged.
...
def _excluded(name: str) -> bool:
    """True if the artifact name matches the exclude list."""
...
def find_orphans(pipelines_path: str = ".", full_traverse: bool = False):
    ...
    all_models = set(meta.models()["Model Group"])
    all_endpoints = set(meta.endpoints()["Name"])

    orphans = {
        "data_sources": sorted(n for n in all_ds - live["ds"] if not _excluded(n)),
        "feature_sets": sorted(n for n in all_fs - live["fs"] if not _excluded(n)),
        "models": sorted(n for n in all_models - live["model"] if not _excluded(n)),
    }
    ...
    # Ordered endpoint -> model -> fs -> ds to respect the dependency chain.
    tuples = []
    for key in ("endpoints", "models", "feature_sets", "data_sources"):
