"""CYP PyTorch Tabular UQ models on 2D + curated xTB 3D-v2 features — one per isoform.

The PyTorch tabular model is single-target, so each of the four scored isoforms gets
its own model off the shared `openadmet_cyp_f2` FeatureSet (featurized through `smiles-to-2d-3d-v2` =
...
VARIANT = "2d-3dv2"
BASE_TAGS = ["openadmet_cyp", "pytorch", "regression", "3dv2"]
