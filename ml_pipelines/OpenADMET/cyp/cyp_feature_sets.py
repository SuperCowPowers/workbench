...
Variants differ only in the 3D layer, and `openadmet_cyp_f2` (smiles-to-2d-3d-v2,
the curated xTB layer) is commented out of `FEATURE_ENDPOINTS` — that pass runs
xTB over every compound. Uncomment it to build the 3D ensemble member; the
InferenceCache is SMILES-keyed and S3-persisted, so it is paid once.

Three FeatureSets, matching the challenge's two scored tracks plus pretraining:

  - openadmet_cyp_f1         Regression track. 4,905 compounds, four sparse pIC50
                             targets, 2D + v1 3D features. Each target carries
                             `_ci_lower`/`_ci_upper` (what
                             `compute_regression_metrics` reads to emit `st_rae`)
                             and `_std`.
  - openadmet_cyp_tdi_f1     TDI track. 6,145 compounds, `cyp3a4_is_tdi` and
...
`compute_regression_metrics` finds the credible interval by appending `_ci_lower`
to the target name, so renaming the targets silently drops ST-RAE.
...
FEATURE_ENDPOINTS = {
    "openadmet_cyp_f1": "smiles-to-2d-3d-v1",  # v1 3D (74 descriptors)
    # "openadmet_cyp_f2": "smiles-to-2d-3d-v2",  # v2 3D (curated, xTB) — the long pole
}
...
for fs_name, endpoint in FEATURE_ENDPOINTS.items():
    build_feature_set(fs_name, endpoint, inhibition, tags=["openadmet_cyp", "multi_task", "activity"])
...
build_feature_set("openadmet_cyp_tdi_f1", "smiles-to-2d-3d-v1", tdi, tags=["openadmet_cyp", "tdi", "classification"])
