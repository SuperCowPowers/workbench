"""Open ADMET logd: hyperparameter-searched Chemprop, alongside the hand-tuned one.

Same FeatureSet, target and held-out capture as the `logd-reg-chemprop` model in logd.py —
...
logd is the cleanest read available on whether HPO earns its keep:

  - 5039 rows, the largest of the assays after ksol
  - std 1.19 over a -2.0 to 5.2 range, the widest dynamic range in the set
  - the only assay with no transform and no pile-up at a censoring floor, so the search
...
capture — 2282 molecules that are not in the FeatureSet at all — compared against the same
capture from the untuned `logd-reg-chemprop`.
...
TARGET = "logd"
MODEL_NAME = "logd-reg-chemprop-hpo"
