...
class FeatureSetCore(Artifact):
    ...
    def create_data_quality_view(self):
        """Create a view in Athena that adds data quality columns for this FeatureSet"""

        # Create the view name
        view_name = f"{self.athena_table}_dq_view"
        self.log.important(f"Creating Data Quality View {view_name}...")

        # Temp: Randomly assign a data quality score from 0 to 1
        create_view_query = f"""
        CREATE OR REPLACE VIEW {view_name} AS
        SELECT *, RAND() AS dq_score
        FROM {self.athena_table}
        """
    ...
    def get_view_tablename(self, view_type: str, create: bool = True) -> Union[str, None]:
        """Get the specified view table for this FeatureSet
        Args:
            view_type (str): The type of view to get (e.g., "training", "data_quality")
        ...
        """
        if view_type not in ["training", "data_quality"]:
            raise ValueError(f"Invalid view_type: {view_type}")

        # Get the view table name
        view_tablename = f"{self.athena_table}_{view_type}"
        glue_client = self.boto_session.client("glue")
        try:
            glue_client.get_table(DatabaseName=self.athena_database, Name=view_tablename)
            return view_tablename
        except glue_client.exceptions.EntityNotFoundException:
            if not create:
                return None
            self.log.warning(f"View: {view_tablename} for {self.uuid} doesn't exist, creating one...")
            self.create_view(view_tablename, view_type)
            time.sleep(1)  # Give AWS a second to catch up
            return view_tablename

    def delete_view(self, view_type: str):
        """Delete the specified view for this FeatureSet

        Args:
            view_type (str): The type of view to delete (e.g., "training", "data_quality")
        """
        # Get the view table name
        view_tablename = f"{self.athena_table}_{view_type}"
        try:
            view_tablename = self.get_view_tablename(create=False)
            if view_tablename is not None:
                self.log.info(f"Deleting View {view_tablename} for {self.uuid}")
                glue_client = self.boto_session.client("glue")
                glue_client.delete_table(DatabaseName=self.athena_database, Name=view_tablename)
        except botocore.exceptions.ClientError as error:
            # For ResourceNotFound/ValidationException, this is fine, otherwise raise all other exceptions
            if error.response["Error"]["Code"] in ["ResourceNotFound", "ValidationException"]:
                self.log.warning(f"View {view_tablename} for {self.uuid} doesn't exist, nothing to delete...")
                pass
            else:
                raise error
...
if __name__ == "__main__":
    ...
    # Test creating a default data quality view
    print("Creating default data quality view...")
    my_features.create_data_quality_view()
