"""End-to-end AWS smoke: PyTorch tabular with a designated validation set.
...
PyTorch held-out path re-runs the fitted scaler/mappings on the held-out rows.
"""
...
@pytest.mark.long
def test_pytorch_validation_set():
    fs = FeatureSet("abalone_features")
    id_column = fs.id_column
    validation_ids = fs.pull_dataframe()[id_column].tolist()[:50]

    name = "abalone-pytorch-validation-test"
    model = fs.to_model(
        name=name,
        ...
        description="Abalone PyTorch (validation-set smoke)",
