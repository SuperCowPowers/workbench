...
if __name__ == "__main__":
    ...
    if recreate or not Endpoint("tautomerize-v0-rt-fast1").exists():
        m = Model("tautomerize-v0")
        m.set_owner("BW")
        end = m.to_endpoint(name="tautomerize-v0-rt-fast1", tags=["smiles", "tautomerization", "realtime"],
                            serverless=False, instance="ml.c7i.large")
    ...
    if recreate or not Endpoint("smiles-to-md-v0-rt").exists():
        ...
        m.set_owner("BW")
        end = m.to_endpoint(name="smiles-to-md-v0-rt", tags=["smiles", "molecular descriptors", "realtime"], serverless=False)
