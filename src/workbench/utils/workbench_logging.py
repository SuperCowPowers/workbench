...
def _suppress_sagemaker_logging():
    """Suppress SageMaker SDK logging and remove any RichHandlers from root.

    The SageMaker SDK installs a RichHandler on the root logger via
    logging.basicConfig(), which can corrupt output from other loggers.
    """
    logging.getLogger("sagemaker").setLevel(logging.WARNING)

    # Remove any RichHandlers that SageMaker installed on the root logger
    root = logging.getLogger()
    root.handlers = [h for h in root.handlers if "RichHandler" not in type(h).__name__]
