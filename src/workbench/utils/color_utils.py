...
def render_markdown(text: str) -> None:
    ...
    Anything not named below keeps rich's own default (lists and tables in cyan,
    links in bright_blue) rather than a palette color. Follows the active theme,
    since the palette is read per call.
    """
    try:
    ...
    # Fenced blocks render through a Syntax object, which ignores the theme above --
    # "ansi_dark" drops the boxed background and follows the terminal instead.
    Console(theme=theme).print(Markdown(text, code_theme="ansi_dark"))
