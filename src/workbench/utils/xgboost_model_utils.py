...
from workbench.utils.model_utils import load_category_mappings_from_s3, pull_oof_predictions, safe_extract_tarfile
...
def pull_cv_results(workbench_model: Any) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    that held it out, so these are leak-free single-model predictions. Scikit-learn
    models train on a single split and have no out-of-fold rows, so they return empty.
    ...
    if predictions_df.empty:
        log.warning(f"No out-of-fold predictions for {workbench_model.name} (single-split model)")
