...
# Workbench Imports
from workbench.utils.job_tracker import register, report, shutdown
...
def _watch(name: str, interval: int) -> None:
    ...
    while not shutdown.wait(interval):
        ...
        if job_name is None:
            ...
            if df.empty:
                ...
                if waited >= APPEAR_TIMEOUT:
                    row = {"kind": "Batch job", "name": name, "status": "NOT_FOUND"}
                    report({**row, "reason": "never reached the Batch queue"}, success=False)
                    return
                continue
        ...
        if row["status"] in _TERMINAL_STATUS:
            report({**row, "kind": "Batch job"}, success=row["status"] == "SUCCEEDED")
            return
