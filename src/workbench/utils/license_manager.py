"""Internal: Workbench API License Manager (used by ConfigManager, do not use directly)"""
...
class LicenseManager:
    ...
    def load_api_license(
        ...
        """Internal: Load the Workbench API License, verify it, and return the licensed features
        Args:
            aws_account_id(str): The AWS Account ID to verify the license against (None for Open Source)
            api_key(str): The Workbench API Key to verify (base64 encoded)
            license_api_key(str): The Workbench License API Key (default: None)
        Returns:
            dict/None: The Workbench API License Information or None if the license is invalid
        """
        ...
        try:
            ...
            cls.log.critical("Please contact Workbench support")
            raise FatalLicenseError()

        # Verify the signature of the API Key
        if not cls.verify_signature(_license_data, signature):
            msg = "API License key verification failed, Please contact Workbench support"
        ...
        if api_account_id and aws_account_id and api_account_id != aws_account_id:
            cls.log.critical("Workbench API Key is not valid for this AWS Account!")
    ...
    def print_license_info(cls):
        ...
        cls.log.important(f"Workbench License: {id}-{account}-{expires}")
