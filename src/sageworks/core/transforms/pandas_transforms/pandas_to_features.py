...
class PandasToFeatures(Transform):
    ...
    def one_hot_encode(self, df, one_hot_columns: list) -> pd.DataFrame:
        ...
        # Now convert the list of columns into One Hot Encoding
    ...
    def prep_dataframe(self):
        ...
        # Convert object and string types to Categorical
        self.convert_columns_to_categorical()

        # If one-hot columns are provided, convert them to one-hot encoding
...
if __name__ == "__main__":
    ...
    df_to_features.set_output_tags(["test", "small", "one-hot"])
    df_to_features.transform()
    ...
    df_to_features = PandasToFeatures("test_features")
    df_to_features.set_input(data_df, id_column="id")
    df_to_features.convert_columns_to_categorical()
