"""PXR 3D activity models — XGBoost UQ + PyTorch UQ on Boltzmann-ensemble 3D features.

Consumes fs:openadmet_pxr_activity_3d (built by pxr_feature_sets.py).

Models / Endpoints:
    - pxr-3d-reg-xgb              XGBoost UQ, full 3D feature list
    - pxr-3d-reg-pytorch-<N>      PyTorch UQ, all N features
    - pxr-3d-reg-pytorch-100      PyTorch UQ, top-100 non-zero SHAP (from the XGB model)
...
def feature_list() -> list[str]:
    """Full 3D feature list."""
    return list(Endpoint(ENDPOINT_3D).output_columns())


def phase1_test_df():
    """Held-out Analog Set 1, featurized through the cached 3D endpoint."""
