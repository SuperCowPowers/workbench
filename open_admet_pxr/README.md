...
This directory holds the model DAG as separate, self-contained scripts plus a
...
- `pxr_feature_sets.py` — DataSource + `_2d` / `_3d` / `_2d_3d` FeatureSets
- `pxr_2d.py`, `pxr_3d.py`, `pxr_2d_3d.py` — XGBoost UQ + PyTorch UQ per feature block
- `pxr_chemprop.py` — Chemprop D-MPNN (SMILES only)
- `pipelines.json` — DAG manifest (fs producer → model consumers)
