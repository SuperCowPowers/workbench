...
<table style="width: 100%;">
  ...
  <tbody>
    ...
    <tr><td class="text-teal" style="padding: 8px 16px; font-weight: bold;">smiles-to-2d-salt-v1</td><td style="padding: 8px 16px;">~315 2D descriptors</td><td style="padding: 8px 16px;">Solubility only &#8212; keeps the counterion, which solubility actually measures</td></tr>
...
The 2D and 3D endpoints come pre-combined as a [MetaEndpoint](../models/meta_endpoints.md#featured-smiles-to-2d-3d-v2-2d-3d-in-one-call) — **`smiles-to-2d-3d-v2`** (2D + curated 3D v2, ~339 features) — fanning out to both children and concatenating the results into a single descriptor set covering topological, electronic, and geometric properties, in one `inference()` call with no juggling two endpoints. See the [3D Descriptors](3d_descriptors.md) blog for the underlying pipeline.
...
!!! tip inline end "Combine 2D + 3D"
    Use the pre-deployed [MetaEndpoint](../models/meta_endpoints.md#featured-smiles-to-2d-3d-v2-2d-3d-in-one-call) **`smiles-to-2d-3d-v2`** (2D + curated 3D v2, ~339 features) — it fans out to both endpoints and concatenates the results in a single `inference()` call.
...
Our [3D endpoint](3d_descriptors.md) runs RDKit's ETKDGv3 generation, **GFN2-xTB** semi-empirical quantum energy ranking, and Boltzmann-weighted ensemble averaging. **`smiles-to-3d-v2`** emits a curated 26-feature set that harvests quantum electronic descriptors (dipole, frontier orbitals, xTB partial charges) from the same ranking single-point — every feature chosen to add signal orthogonal to 2D. See the [3D Descriptors](3d_descriptors.md) blog for the pipeline and the deprecated 74-feature v1 set.
