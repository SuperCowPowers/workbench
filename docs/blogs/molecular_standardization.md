...
These standardization and descriptor computations run inside Workbench **feature endpoints** — SageMaker-hosted transformer models that take raw SMILES and return standardized structures plus computed descriptors:

- **`smiles-to-2d-v1`**: Standard 2D pipeline with salt extraction (ChEMBL default) — the default for every assay
- **`smiles-to-2d-salt-v1`**: 2D pipeline that preserves salt forms, for solubility work where the counterion is part of what was measured
- **`smiles-to-3d-v2`**: 3D conformer-based descriptors (26 curated GFN2-xTB features), async mode with adaptive Boltzmann conformers

The 2D endpoints can be deployed as serverless (cost-efficient for intermittent workloads) or on dedicated instances for higher throughput; the 3D endpoint is async with scale-to-zero.
