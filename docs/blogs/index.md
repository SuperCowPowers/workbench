...
- **[Do 3D Descriptors Help on PXR Induction?](pxr_2d_vs_3d.md):** A single-target case study — 2D / 3D / 2D+3D ablations (plus a Chemprop-hybrid test) on the OpenADMET PXR induction blind challenge. The twist: on *this* target the 3D conformer features earn high SHAP importance inside the combined model, yet adding them hurts held-out accuracy and a 3D-only model scores *negative* R² on a new analog series. We use the CV-vs-held-out gap to separate features that *fit* from features that *transfer* — and we're careful to scope the result to this one endpoint, not 3D in general.
