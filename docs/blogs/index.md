...
- **[A Weekend on the OpenADMET PXR Challenge](pxr_weekend_experiments.md):** A humble tour of negative results. Over one weekend we threw four increasingly fancy ideas at PXR induction — 2D-vs-3D descriptor ablations, a from-scratch xTB 3D-descriptor rebuild, a multi-model ensemble, and a CheMeleon foundation-model warm-start — and a plain from-scratch Chemprop D-MPNN beat every one of them on the revealed held-out analog series. The recurring lesson: a strong simple baseline plus honest out-of-distribution evaluation is hard to beat, and added complexity mostly hurt transfer. Includes the SHAP "high-importance ≠ generalizes" paradox that explains why.
