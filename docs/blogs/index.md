...
- **[A Weekend on the OpenADMET PXR Challenge](pxr_weekend_experiments.md):** Four experiments against a plain from-scratch Chemprop D-MPNN on PXR induction. Three added complexity for its own sake — an xTB 3D-descriptor rebuild, a multi-model ensemble, a CheMeleon foundation-model warm-start — and all three lost on the revealed held-out analog series. The fourth, multi-task learning on logD/logP (PXR's lipophilicity driver), was the one that finally beat the baseline. The lesson: a strong simple baseline plus honest OOD evaluation is hard to beat, and complexity only pays off when it's aligned with the mechanism.
...
- **[HPO: WIP](hpo_winners_curse.md):** *(work in progress)* A hyperparameter search reports the minimum over many noisy estimates, so its top trial is disproportionately lucky rather than good. Across five real searches, that top trial survived re-scoring exactly once — the configuration we published was usually the leaderboard's second or third. We cover the two-stage selection that catches it, why carrying your own hyperparameters through the second stage bounds the downside, and why cross-validated gains didn't transfer to a held-out analog series.
