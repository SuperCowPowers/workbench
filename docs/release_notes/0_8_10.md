...
- **Display/Training/Computation Views**
	...
	```
    set_display_columns() -> gone
	num_display_columns() -> gone
    set_computation_columns() -> gone
    num_computation_columns() -> gone
    ```

    **Note**: We're still going to have the `fs.set_computation_columns()` method since that handles the underlying recomputation of statistics for the new columns... 
