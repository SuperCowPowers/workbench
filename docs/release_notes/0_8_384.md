...
* **Full endpoint only.** xTB adds ~0.1–0.5 s per conformer; it runs on the async full endpoint where the per-molecule budget accommodates it. The `smiles-to-3d-fast-v1` endpoint is now **deprecated** — use the full endpoint.
