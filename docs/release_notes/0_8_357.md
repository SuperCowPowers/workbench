...
* **`model.default_inference_run()`** is the single resolver for the default inference capture (`full_cross_fold → test_inference → first run`). The `get_inference_metrics` / `confusion_matrix` sentinel is now `"default"` (was `"auto"`).
