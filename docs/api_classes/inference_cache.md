...
`InferenceCache` is a caching wrapper around a Workbench `Endpoint`. It's handy when an endpoint is slow to invoke and the same inputs show up across calls — the motivating example is the 3D molecular feature endpoint `smiles-to-3d-v2`, which takes real time to generate conformers and force-field optimize each molecule.
...
```
InferenceCache[smiles-to-3d-v2]: 0/50 cache hits
InferenceCache[smiles-to-3d-v2]: computing 50 new rows via endpoint
InferenceCache[smiles-to-3d-v2]: 50/50 cache hits
InferenceCache[smiles-to-3d-v2]: removed 1 entries
InferenceCache[smiles-to-3d-v2]: removed 3 entries
```
