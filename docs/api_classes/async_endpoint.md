...
endpoint = Endpoint("smiles-to-3d-v2")
...
model = Model("smiles-to-3d-v2")
model.to_endpoint(async_endpoint=True, tags=["smiles", "3d descriptors", "full"])
```

## Async Meta Endpoints

An async endpoint can itself be a *composition* of other endpoints. **[`smiles-to-2d-3d-v2`](../models/meta_endpoints.md#featured-smiles-to-2d-3d-v2-2d-3d-in-one-call)** is a [MetaEndpoint](../models/meta_endpoints.md) that fans out to the sync 2D descriptor endpoint and the async 3D descriptor endpoint, then concatenates the results into one ~339-feature DataFrame:
...
Because one child (`smiles-to-3d-v2`) is async, the whole MetaEndpoint is **auto-deployed async** — the caller sees a single endpoint and a single `inference()` call, with the S3 round-trip and per-child sync/async dispatch handled server-side.
