...
def on_page_load():
    """Two-way deep link: ?name= selects a feature set, selecting one updates the URL"""
    sync_selection("feature_sets_table", "feature_sets_page_loaded")
...
def update_feature_set_details(feature_details: DataDetails):
    ...
    def generate_feature_set_markdown(selected_rows, _theme):
...
def update_feature_set_sample_rows(samples_table: AGTable):
    @callback(
        [
            ...
            Output("feature_set_sample_rows_original", "data"),
...
def reorder_sample_rows():
    ...
    @callback(
        ...
        Input("feature_set_violin_plot", "selectedData"),
        State("feature_set_sample_rows_original", "data"),
        prevent_initial_call=True,
    )
