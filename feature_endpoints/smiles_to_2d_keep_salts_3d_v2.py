...
keeps salts (for assays like solubility where the salt form matters):
    ...
    [smiles-to-2d-keep-salts-v1]  (sync, RDKit + Mordred 2D, salts kept)  ──┐
                                                                            ├── Concat
...
Created artifacts:  Model/Endpoint ``smiles-to-2d-keep-salts-3d-v2``
"""
...
ENDPOINT_NAME = "smiles-to-2d-keep-salts-3d-v2"
TAGS = ["smiles", "2d", "keep-salts", "3d", "v2", "meta"]
...
if __name__ == "__main__":
    ...
    dag = MetaEndpointDAG()
    dag.add_endpoint("smiles-to-2d-keep-salts-v1")
    dag.add_endpoint("smiles-to-3d-v2")
    dag.add_aggregation(Concat(name="combine"))
    dag.add_edge("smiles-to-2d-keep-salts-v1", "combine")
    dag.add_edge("smiles-to-3d-v2", "combine")
    dag.set_input_node("smiles-to-2d-keep-salts-v1", "smiles-to-3d-v2")
