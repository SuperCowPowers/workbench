...
if __name__ == "__main__":
    ...
    # Deploy the endpoint too (serverless, so it scales to zero)
    if not aws_model.endpoints():
        aws_model.to_endpoint(name="local-test-regression-end")

    # Compare out-of-fold predictions: same script, same rows, same roles
    local_oof = model.oof_predictions()
    aws_oof = Model("local-test-regression").get_inference_predictions()

    if aws_oof is None:
        log.warning("No AWS inference predictions yet, skipping the comparison")
        raise SystemExit(0)
